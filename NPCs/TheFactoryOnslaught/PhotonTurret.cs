...
{
    public class PhotonTurret : ModNPC
    {
        int fireTimer;
        int fireTimerMax = 60;
        ...
        public override void AI()
        {
            ...
            if (fireTimer <= fireTimerMax)
            {
                if (fireTimer % 30 == 0)
                {
                    ...
                    {
                        Vector2 vector = (player.Center - NPC.Center).RotatedByRandom(MathHelper.ToRadians(5));
                        vector.Normalize();
                        vector *= 32f * Main.rand.NextFloat(0.5f, 0.8f);
                        Projectile.NewProjectileDirect(NPC.GetSource_FromThis(), NPC.Center, vector,
                            ModContent.ProjectileType<PhotonLaser>(), 28, 3, Main.myPlayer);
